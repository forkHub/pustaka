...
export class PathFinder {
    ...
    private cariCellTerdekatKeTarget(): PFCell | undefined {
        let jarakTerdekat: number = Number.MAX_SAFE_INTEGER;
        let jarakSementara: number = 0;
        let cellRes: PFCell | undefined = undefined;

        for (const  cell of this._cells) {
            jarakSementara = Math.abs(cell.x - this._targetX) + Math.abs(cell.y - this._targetY);
    ...
    private cellCreate(parent: PFCell | null, i: number, j: number): PFCell {
        let cell: PFCell;
        ...
        let distG = Math.abs(this._startX - i) + Math.abs(this._startY - j);
        let distH = Math.abs(this._targetX - i) + Math.abs(this._targetY - j);
        cell.dist = distG * 1.5 + distH;
    ...
    search(sx: number, sy: number, tx: number, ty: number): Array<{x:number, y:number}> {
    ...
    private cellOpen(cellCr: PFCell): void {
        //up
        if (this.cellPosPossible(cellCr.x, cellCr.y - 1)) {
            this._cells.push(
                this.cellCreate(cellCr, cellCr.x, cellCr.y - 1)
            );
        }
        ...
        if (this.cellPosPossible(cellCr.x + 1, cellCr.y)) {
            this._cells.push(
                this.cellCreate(cellCr, cellCr.x + 1, cellCr.y)
            );
        }
        ...
        if (this._flDiagonal) {
            ...
            if (this.cellPosPossible(cellCr.x + 1, cellCr.y - 1)) {
                this._cells.push(
                    this.cellCreate(cellCr, cellCr.x + 1, cellCr.y - 1)
                );
            }
    ...
    private getPathImpl(): Array<PFCell> {
        let cellCr: PFCell | undefined;
        let res: Array<PFCell> = new Array<PFCell>();

        if (this._startX == this._targetX && this._startY == this._targetY) {
            return res;
        }

        //cell pertama
        this._cells.push(this.cellCreate(null, this._startX, this._startY));
        ...
        while (true) {
            if (this._cells.length >= this._maxCells) {
                if (this._flBlocked == PathFinder.BL_STOP) {
                    ...
                    cellCr = this.cariCellTerdekatKeTarget();
                } else {
                    throw new Error();
                }
            ...
            if (cellCr) {
                ...
                if (this.checkSampaiTujuan(cellCr.x, cellCr.y, this._targetX, this._targetY)) {
                    res.unshift(cellCr);
