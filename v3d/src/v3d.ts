...
// Rotation helpers
function rotateX([x, y, z]: Vertex3D, angle: number): Vertex3D {
	const cos = Math.cos(angle), sin = Math.sin(angle);
	return [x, y * cos - z * sin, y * sin + z * cos];
}
function rotateY([x, y, z]: Vertex3D, angle: number): Vertex3D {
	const cos = Math.cos(angle), sin = Math.sin(angle);
	return [x * cos + z * sin, y, -x * sin + z * cos];
}
function rotateZ([x, y, z]: Vertex3D, angle: number): Vertex3D {
	const cos = Math.cos(angle), sin = Math.sin(angle);
...
function projectVertices(
	...
	cameraDistance: number = 1000,
	cameraRotation: Vertex3D = [0, 0, 0],
	cameraPosition: Vertex3D = [0, 0, 0]
): { projected: Point2D[], transformed: Vertex3D[] } {
...
function computeNormal(vertices: Vertex3D[], face: number[]): Vertex3D {
	const [i0, i1, i2] = face;
...
// Dot product
function dot(a: Vertex3D, b: Vertex3D): number {
	return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
}

// Edge length in 3D
function edgeLength(a: Vertex3D, b: Vertex3D): number {
...
function renderFaces(
	...
	projected: Point2D[],
	transformed: Vertex3D[],
	faces: number[][],
	cameraDir: Vertex3D = [0, 0, -1]
) {
