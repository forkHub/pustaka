...
let data: String[];
let txtLn: number = 10; //TODO: delete
...
function Update() {
	...
	if (KeyboardIsDown("ArrowLeft")) {
		char.x -= velX;
		if (getCollidedTile()) char.x = collidedTile.x + 32;
	}
...
function getCollidedTile(): boolean {
	collidedTile = null;
...
function debug() {
	...
	if (collidedTile) WriteLn("collided tile " + collidedTile.x + "/" + collidedTile.y);
}
